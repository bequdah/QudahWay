...
3. لا تذكر أسماء معدّي المواد إلا لو سألوك صراحة.
...
8. معلومات هامة عن إعداد المواد (اذكرها فقط عند السؤال):
   - مادة علم البيانات (DS) تم إعدادها من قبل "د. ملك العبدالله".
   - مادتي استرجاع المعلومات (IR) والرؤية الحاسوبية (CV) تم إعدادهما من قبل "د. عبدالله العمارين".
...
export async function onRequestPost({ request, env }) {
    ...
    try {
        ...
        return new Response(JSON.stringify({
            answer: "صارت مشكلة تقنية: " + (err.message || String(err)) + " | Stack: " + (err.stack || "No stack")
        }), { status: 200, headers: corsHeaders });
    }
}
